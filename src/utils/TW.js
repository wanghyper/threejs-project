...
TW.cameraSetup = function (renderer, scene, scene_bb, inits) {
    ...
    if (ah == null) {
        throw "Your scene has no AxisHelper, which means it wasn't created by " + 'TW.mainInit().';
    }
...
TW.createWireBarn = function (width, height, length, lineWidth) {
    ...
    */
    ...
    var mat = new THREE.LineBasicMaterial({color: 0xffffff, vertexColors: THREE.FaceColors, linewidth: lw});
    var barnObj = new THREE.LineSegments(geometry, mat);

    var parent = new THREE.Object3D();
    parent.add(barnObj);
    var origin = new THREE.Mesh(new THREE.SphereGeometry(0.4, 8, 8), new THREE.MeshBasicMaterial({color: 0xffffff}));
    parent.add(origin);
    return parent;
};
...
TW.createFrustum = function (left, right, top, bottom, near, far) {
    ...
    var geometry = new THREE.Geometry();
    var material = new THREE.LineBasicMaterial({color: 0xffffff, linewidth: 3, vertexColors: THREE.FaceColors});
...
TW.convertMousePositionToNDC = function (event) {
    ...
    var clickInfo = {mx: mx, my: my, cx: cx, cy: cy, winXSize: winXSize, winYSize: winYSize, x: x, y: y};
    return clickInfo;
};
...
TW.createUSFlagGray = function (log2size) {
    ...
    for (i = 0; i < unionHoist; i++) for (j = 0; j < unionFly; j++) TW.rowMajorAset(data, i, j, BLUE);
    return data;
};
...
TW.rowMajorArefRGB = function (arrayRGB, i, j) {
    ...
    if (j > width) throw 'array col out of bounds: ' + j + ' > ' + width;
    var index = i * width + j;
    return [arrayRGB[3 * index + 0], arrayRGB[3 * index + 1], arrayRGB[3 * index + 2]];
};
...
TW.createUSFlagRedWhiteBlue = function (log2size) {
    ...
    var unionHoist = 7 * stripe_height;
    var unionFly = Math.round(0.76 * hoist); // from flag spec
    for (i = 0; i < unionHoist; i++) for (j = 0; j < unionFly; j++) TW.rowMajorAsetRGB(data, i, j, 0, 0, 0xff); // BLUE
    return data;
};
...
TW.textureMaterial = function (url) {
    ...
    // return it.
    numTexturesToLoad++;
    var texture = new THREE.ImageUtils.loadTexture(url, new THREE.UVMapping(), function (text) {
        console.log('loaded: ' + url);
        if (!TW.isPowerOf2(text.image.width) || !TW.isPowerOf2(text.image.height)) {
            throw 'Image dimensions are not powers of two: ' + url + ': ' + text.image.width + 'x' + text.image.height;
        }
        numTexturesToLoad--;
...
THREE.BezierSurfaceGeometry = function (controlPoints, sSegments, tSegments) {
    ...
    this.parameters = {controlPoints: controlPoints, sSegments: sSegments, tSegments: tSegments};
...
THREE.BezierSurfaceGeometry.prototype.computeBezierSurfacePoints = function () {
    ...
    if (TW.debug) console.log('computing Bezier surface vertices with: ' + sSegments + ' and ' + tSegments);
...
THREE.BezierSurfaceGeometry.prototype.computeBezierSurfaceFaces = function () {
    ...
    for (iz = 0; iz < gridZ; iz++) {
        for (ix = 0; ix < gridX; ix++) {
            ...
            if (TW.debug) console.log('vertex indices:\na: ' + a + '\nb: ' + b + '\nc: ' + c + '\nd: ' + d);

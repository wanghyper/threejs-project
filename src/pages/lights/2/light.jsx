...
import sm_vert from './sm_vert.glsl';
import sm_frag from './sm_frag.glsl';
export default function Light() {
    // https://juejin.cn/post/6956841907372687374
    useEffect(() => {
        let renderer;
        let stats;
        let camera;
        ...
        function initScene() {
            ...
            //渲染目标缓冲区
            bufferTexture = new THREE.WebGLRenderTarget(domElement.clientWidth, domElement.clientHeight);
        ...
        function initCamera() {
            ...
            scene.add(camera);
            // 暂时不知道为啥不能使用透视相机
            camera4SM = new THREE.PerspectiveCamera(45, width / height, 1, 1000);
            ...
            const controls = new OrbitControls(camera4SM, renderer.domElement);
            controls.update();

            const lightCameraHelper = new THREE.CameraHelper(camera4SM);
        ...
        function initObject() {
            ...
            const getSMMaterial = new THREE.ShaderMaterial({
                uniforms: {
                    cameraNear: {value: camera4SM.near},
                    cameraFar: {value: camera4SM.far},
                    projectionMatrixSM: {value: camera4SM.projectionMatrix},
                },
                vertexShader: sm_vert,
                fragmentShader: sm_frag,
            });
